...
to setup
  ...
  set radius-sheep 2
  set radius-shepherd 65
  set num-neighbors round (.53 * num-sheep)
  ...
  set weight-wall -1
  set shepherd-model "strombom"

  set-default-shape sheep "sheep"
  set-default-shape shepherds "turtle"
  ...
  set fN radius-sheep * (count sheep) ^ (2 / 3)
...
to check-win
  let global-com gcm
  let cohesive? not any? sheep with [distancexy (vx global-com) (vy global-com) > fN]
  let at-goal? norm (gcm matrix:- (vec2 dest-x dest-y)) < goal-tolerance
...
to-report average-spread-global
  let center gcm
  report mean [norm (center matrix:- pos)] of sheep
end

to-report average-distance-from-goal
  let dest vec2 dest-x dest-y
  report mean [norm (pos matrix:- dest)] of sheep
end

to-report time-trial-step
  report (list average-spread-global average-distance-from-goal ticks)
end
...
to-report force-other-shepherds
  let away-from-shepherds [([pos] of myself) matrix:- pos] of other shepherds in-radius shepherd-r
  report weight-r-sheep matrix:* normalize(sum-vec2 (map [x -> 1.0 / (norm x) ^ 2 matrix:* x] away-from-shepherds))
end
